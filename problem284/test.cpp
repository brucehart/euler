...
void generate_automorphic_numbers(int base, int digits)
{
    ...
    for (int r = 0; r < 2; ++r)
    {
        ...
        for (int k = 1; true; ++k)
        {
            ...
            boost::multiprecision::cpp_int log14_nk = 0;
            boost::multiprecision::cpp_int temp_nk = n_k;
            while (temp_nk >= base)
            {
                temp_nk /= base;
                log14_nk++;
            }
            if (log14_nk >= 10001 || n_k == 0)
            {
                std::cout << "log14(n_k) >= 10001 at k=" << k << ", stopping computation." << std::endl;
                std::string n_base14 = "";
            while (temp > 0)
            {
...
{
    int base = 14;
    int digits = 50; // You can adjust this to generate more digits
